...
TEST(io, flags_bitwise_or)
{
    ...
    EXPECT_TRUE((f | io::flags::out).test(io::flags::inout));
    EXPECT_TRUE((io::flags::out | f).test(io::flags::inout));

    f = io::flags::out;
    EXPECT_TRUE((f | io::flags::out).test(io::flags::out));
    EXPECT_TRUE((io::flags::out | f).test(io::flags::out));

    EXPECT_TRUE((f | io::flags::in).test(io::flags::inout));
    EXPECT_TRUE((io::flags::in | f).test(io::flags::inout));
    ...
    f = io::flags::error | io::flags::in;
    EXPECT_TRUE(f.test(io::flags::error));
    EXPECT_TRUE(f.test(io::flags::in));

    f = io::flags::in | io::flags::error;
    EXPECT_TRUE(f.test(io::flags::error));
    EXPECT_TRUE(f.test(io::flags::in));

    f = io::flags::error | io::flags::out;
    EXPECT_TRUE(f.test(io::flags::error));
    EXPECT_TRUE(f.test(io::flags::out));

    f = io::flags::out | io::flags::error;
    EXPECT_TRUE(f.test(io::flags::error));
    EXPECT_TRUE(f.test(io::flags::out));
}

TEST(io, flags_bitwise_assign_or)
{
    io::flags f{io::flags::empty};
    EXPECT_TRUE(f.test(io::flags::empty));
